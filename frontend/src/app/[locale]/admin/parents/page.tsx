...
export default function ParentsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get("/admin/parents", { params: { page, page_size: 10, search: debouncedSearch } }).then((r: any) => r.data),
  });

  // Summary
  const { data: allData } = useQuery({
    queryKey: ["parents-summary"],
    queryFn: () => api.get("/admin/parents", { params: { page_size: 500 } }).then((r: any) => r.data),
  });
  const allParents: any[] = allData?.data ?? [];
  const totalChildren = allParents.reduce((s, p) => s + (p.students?.length ?? 0), 0);

  const del = useMutation({
    mutationFn: (id: string) => api.delete(`/admin/parents/${id}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["parents"] });
      qc.invalidateQueries({ queryKey: ["parents-summary"] });
  ...
  const columns = [
    {
      key: "name", label: t("name"), render: (r: any) => (
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center text-purple-700 dark:text-purple-300 text-sm font-bold">
    ...
    {
      key: "children", label: t("children"), render: (r: any) => (
        <button
          onClick={() => setChildrenParent(r)}
          className="badge badge-blue hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors cursor-pointer"
        >
          {r.students?.length ?? 0} {r.students?.length === 1 ? "child" : "children"}
        </button>
      ),
    },
    {
  ...
  return (
    <DashboardLayout role="admin">
      <PageHeader
        ...
        subtitle={t("parentsSubtitle", { count: data?.total ?? 0 })}
        actions={
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <StatCard title="Total Parents"   value={allData?.total ?? 0} icon={Users} color="purple" />
        <StatCard title="Linked Children" value={totalChildren}        icon={Users} color="blue"   />
      </div>
      ...
      <div className="card overflow-hidden">
        ...
        {data && data.total_pages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-[var(--color-border)] dark:border-[var(--color-dark-border)]">
            <p className="text-sm text-[var(--color-text-muted)]">{t("page")} {page} {t("of")} {data.total_pages} · {data.total} {t("results")}</p>
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title={selected ? t("editParent") : t("addParent")}>
        <ParentForm existing={selected} onSuccess={() => {
          setShowModal(false);
          qc.invalidateQueries({ queryKey: ["parents"] });
          qc.invalidateQueries({ queryKey: ["parents-summary"] });
        }} />
        ...
        <Modal open={true} onClose={() => setDeleteTarget(null)} title={t("deleteConfirm")}>
          <div className="space-y-4">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button onClick={() => del.mutate(deleteTarget.id)} disabled={del.isPending} className="px-4 py-2 rounded-xl text-sm font-semibold bg-red-600 hover:bg-red-700 text-white transition-colors">
                {t("delete")}
              </button>
            </div>
          </div>
...
function ChildrenView({ parentId }: { parentId: string }) {
  ...
  if (isLoading) return <div className="py-8 text-center text-sm text-[var(--color-text-muted)]">{t("saving")}</div>;
...
function ParentForm({ existing, onSuccess }: { existing: any; onSuccess: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium mb-1 dark:text-[var(--color-dark-text)]">{t("occupation")}</label>
      ...
      <button type="submit" disabled={loading} className="btn-primary w-full justify-center">
        {loading ? t("saving") : existing ? t("editParent") : t("addParent")}
      </button>
    </form>
  );
}
