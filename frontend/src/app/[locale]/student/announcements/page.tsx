...
export default function StudentAnnouncementsPage() {
  ...
  const { data: announcements = [], isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get("/announcements").then((r: any) => {
        const all: any[] = r.data?.data ?? r.data?.records ?? r.data ?? [];
        return all.filter((a: any) =>
          !a.target_roles || a.target_roles.length === 0 || a.target_roles.includes("student")
        );
      }),
  });
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-[var(--color-text-muted)] mt-2">
                    ...
                    {a.author && ` · ${t("by") ?? "by"} ${a.author.first_name} ${a.author.last_name}`}
                  </div>
                </div>
              </div>
            </motion.div>
